...
### Binary signing (all platforms) — trust v2

Every ntm-server and ntm-client binary is signed at build time as a mandatory
POST_BUILD step into an **NTMSIG 2 bundle** (`<binary>.sig`). An unsigned binary
is never produced: if this host's build key or a root-signed delegation is
missing, the build fails. Design: `docs/signing-trust-model.md`.

```
3 root keys (compiled in, 2-of-3)  ──sign──►  signing/delegation.txt
delegation  ──names──►  linux-build (linux-amd64) · windows-build (windows-amd64)
build key   ──signs──►  binary;  .sig = delegation + root sigs + key id + signature
```

| Item | Location |
|---|---|
| Root public keys | `signing/roots/r{1,2,3}.pub.der` → `src/trust_roots.hpp` (generated by `scripts/trust/gen-roots-header.sh`) |
| Root private seeds | **owner only, offline** (r1 Keychain + SLIP-39 backup; r2, r3 cards). Never on an agent host. |
| Delegation + root sigs | `signing/delegation.txt`, `signing/delegation.txt.rN.sig` (public, committed) |
| linux-build seed | `~/.config/ntm/linux-build.cred` — `systemd-creds --user`, host-bound (Fedora Linux Agent) |
| windows-build seed | `%USERPROFILE%\.ntm\windows-build.dpapi` — DPAPI CurrentUser (Windows Agent; `scripts/trust/windows-build-seed.ps1`) |

| Script | Purpose |
|---|---|
| `scripts/sign-server.sh`, `scripts/sign-client.sh` | CMake POST_BUILD entry points → `scripts/trust/sign-artifact.sh` |
| `scripts/trust/verify-bundle.sh <binary>` | independent CLI check of a `.sig` bundle |
| `scripts/trust/make-delegation.sh`, `root-sign.sh`, `verify-delegation.sh` | delegation lifecycle (`root-sign.sh` is run by the owner only) |

A build key is **never copied between machines** and **never regenerated on an
agent's own initiative** — a lost or rotated build key is a new delegation,
signed by the owner (`docs/signing-trust-model.md §7`).
...
| Build key for the binary's platform | see §9 "Binary signing" | signs the push auth proof; the pushed `.sig` bundle must name the same key |
...
1. Verifies the NTMSIG 2 bundle (2-of-3 roots, delegation expiry and rollback
   floor, `linux-build` key, binary signature)
2. Verifies the ML-DSA-65 auth proof (nonce + binary hash) with that same key
...
For Windows client binaries — run on the **Windows build host** (MSYS2 bash),
because the auth proof must be signed with the `windows-build` key, which never
leaves that host:

```bash
./scripts/push-client.sh build-windows/ntm-client-windows-amd64-<MAJOR.MINOR.PATCH.REVISION>.exe --confirm
```
...
1. Validates filename format and platform
2. Verifies the NTMSIG 2 bundle for that platform (roots, delegation, build key,
   binary signature)
3. Verifies the ML-DSA-65 auth proof with the same delegated build key
...
- **Valid pair**: binary matching `ntm-client-<platform>-<version>[.exe]` AND
  a matching `.sig` NTMSIG 2 bundle that verifies (roots, unexpired delegation
  not older than the server's own, platform-scoped key).
...
Every ntm-client binary verifies its own NTMSIG 2 bundle at startup (Step
0 of `main()`); expiry is not enforced at startup (maintainer ruling, #133). If the `.sig` file is missing or the signature fails, the
...
binary and verifies it — including expiry and a delegation no older than its
own — before applying the update. An update that fails
