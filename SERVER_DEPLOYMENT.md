...
12. [Client auto-update distribution](#12-client-auto-update-distribution)
13. [Resource limits reference](#13-resource-limits-reference)
...
## 12. Client auto-update distribution
...
### Place binaries with the correct naming convention

Binaries must be named exactly as follows for the server to recognise them during scanning:

| Platform | Filename |
|---|---|
| Linux (amd64) | `ntm-client-linux-amd64-<version>` |
| Windows (amd64) | `ntm-client-windows-amd64-<version>.exe` |
...
Files that do not match the naming pattern are ignored during scanning.
...
The `/api/update/*` endpoints are only registered when `update_dir` is set to a non-empty
value. Restart the server after adding this key.

### Scan the manifest

After placing or replacing binaries, trigger a manifest rescan so the server knows what is
...
The manifest is held in server memory. Clients with `auto_update=true` will pick up the
update on their next 23-hour check cycle, or immediately after an operator clicks
**"Force Update"** for a specific client in the admin page.
...
/var/lib/ntm-server/
    updates/
        ntm-client-linux-amd64-1.9.0
        ntm-client-windows-amd64-1.9.0.exe
```

The manifest is built in server memory on startup and on each `/api/admin/update/scan` call.
No manifest file is written to disk.

---

## 13. Resource limits reference
