...
After placing or replacing binaries, regenerate `manifest.json` so the server knows what is
...
The manifest is written to `update_dir/manifest.json`. Clients with `auto_update=true` will
pick up the update on their next 23-hour check cycle, or immediately after an operator clicks
...
/var/lib/ntm-server/
    updates/
        ...
        manifest.json                    # auto-generated by scan
```
